...
local function lstm_attention(x, prev_c, prev_h, m)
  -- Calculate all four gates in one go
  local i2h = nn.Linear(params.rnn_size, 4*params.rnn_size)(x)
  local h2h = nn.Linear(params.rnn_size, 4*params.rnn_size)(prev_h)
  local m2h = nn.Linear(params.rnn_size, 4*params.rnn_size)(m)
  local gates = nn.CAddTable()({i2h, h2h, m2h})
...
local function create_network()
  ...
  for layer_idx = 1, params.layers do
    ...
    local U = nn.Linear3D(params.rnn_size, 1)
    local W1 = nn.Linear3D(params.rnn_size, params.rnn_size*params.max_seq_length)
    local W2 = nn.Linear3D(params.rnn_size, params.rnn_size)
    local f = nn.CAddTable()({
      nn.View(-1, params.max_seq_length, params.rnn_size)(
        W1(nn.View(-1, 1, params.rnn_size)(prev_h))
      ),
      W2(s_encs)
    })
    local v = nn.View(-1, params.max_seq_length)(U(nn.Sigmoid()(f)))
    local a = nn.View(-1, params.max_seq_length, 1)(nn.SoftMax()(v))
    local m = nn.View(-1, params.rnn_size)(nn.MM(true, false)({a, s_encs}))

    local next_c, next_h = lstm_attention(dropped, prev_c, prev_h, m)
...
local function _fp(state)
  ...
  for i = 1, params.max_seq_length do
    ...
    -- table.insert(model.s_encs, model.s_enc[i][2*params.layers])
    if model.s_encs == nil then
      model.s_encs = model.s_enc[i][2*params.layers]:float():view(-1,1,params.rnn_size)
    else
      model.s_encs = torch.cat(
        model.s_encs, model.s_enc[i][2*params.layers]:float():view(-1,1,params.rnn_size), 2
      )
    end
  end

  model.s_encs = transfer_data(model.s_encs)
  ...
  for i = 1, params_sent.max_seq_length do
    ...
    model.err[i], model.s_dec[i] = unpack(
      model.rnns_dec[i]:forward({emb, y, s_dec, model.s_encs})
    )
  end
...
local function _bp(state)
  ...
  for i = params_sent.max_seq_length, 1, -1 do
    ...
    tmp_dec = model.rnns_dec[i]:backward(
      {embeddings, y, s_dec, model.s_encs},
